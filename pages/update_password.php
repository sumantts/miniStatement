<?php 
if(!$_SESSION["User_Id"]){header('location:?p=login');}
include('common/header.php');?>
  <body>
    <div class="container-scroller">
      <!-- partial:partials/_navbar.html -->
      <?php include('common/navbar.php');?>
      <!-- partial -->
      <?php include('common/left_menu.php');?>
        <!-- partial -->
        <div class="main-panel">
          <div class="content-wrapper">
              <div class="col-12 grid-margin">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title"><?=$title?></h4>
                    <form class="form-sample">
                      <!--<p class="card-description"> Personal info </p>-->
                      <div class="row">
                        <div class="col-md-6">
                          <div class="form-group row">
                            <label class="col-sm-4 col-form-label text-danger">Password *</label>
                            <div class="col-sm-8">
                              <input type="password" id="password" class="form-control" />
                              <span class="col-form-label  text-danger" id="password_error" style="font-size: 12px;"></span>
                              <span class="col-form-label  text-success" id="password_success" style="font-size: 12px;"></span>
                            </div>
                          </div>
                        </div>

                        <div class="col-md-6">
                          <div class="form-group row">
                            <label class="col-sm-4 col-form-label text-danger">Confirm Password *</label>
                            <div class="col-sm-8">
                              <input type="password" id="confirm_password" class="form-control" />
                              <span class="col-form-label  text-danger" id="confirm_password_error" style="font-size: 12px;"></span>
                              <span class="col-form-label  text-success" id="confirm_password_success" style="font-size: 12px;"></span>
                            </div>
                          </div>
                        </div>

                        <div class="col-md-6">
                          <div class="form-group row">                            
                          <input type="hidden" name="Staff_Id" id="Staff_Id" value="<?=$_SESSION["Staff_Id"]?>" />
                          <input type="hidden" name="User_Id" id="User_Id" value="<?=$_SESSION["User_Id"]?>" />
                          <button type="button" id="updatePassword" class="btn btn-inverse-success btn-fw">Update</button>
                          </div>
                        </div>
                      </div>
					  
                      <div class="row" id="part_one" style="display: none">
                      <!-- Table start 36409-->
                      <div style="overflow-x: scroll;">
                      <table class="table table-bordered">
                        <thead>
                          <tr style="text-align: center;">
                            <th scope="col" colspan="2" id="text_org_name"> </th>
                          </tr>
                          <tr style="text-align: center;">
                            <th scope="col" colspan="2">Collection Receipt</th>
                          </tr>
                          <tr style="text-align: center;">
                            <th scope="col" colspan="2" id="text_collection_date"> </th>
                          </tr>
                          <tr style="text-align: center;">
                            <th scope="col" colspan="2" id="text_mem_name"> </th>
                          </tr>
                        </thead>
                        <tbody id="collectionList"> </tbody>
                      </table>
                      </div>
                      <!-- Table End -->			
                      </div>

                      <!-- WhatsApp Data Div End --> 
                      <div class="col-md-6" id="receipt_body" style="display: none">
                      </div>
                      <div class="row" id="part_two" style="display: none"> 	
                      <div><img id='button' onClick="shareMe()" src="data:image/png;base64,">

                      </div>	
                      </div> 		
                      <!-- WhatsApp Data Div End -->
                      </div>
                      
                      </div>                     
                      
                      
                    </form>
                  </div>
                </div>
              </div> 		  
		  </div>
          <!-- content-wrapper ends -->
          <!-- partial:partials/_footer.html -->
          <script>
            function shareMe() {
              const button = document.getElementById("button");
              var divblock = $('#receipt_body').text();
              window.open("https://api.whatsapp.com/send?text=" + divblock)
            };

          </script>
          
    <?php include('common/footer.php');?>